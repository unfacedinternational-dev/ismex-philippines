...
/* THE WHITE ON BLACK ENTER BUTTON */
...
if st.session_state.page == "boss_key":
    st.markdown("<style>input[type='password']{color:#0e1117!important;caret-color:transparent!important;width:10px!important;}div[data-testid='stTextInput'] label{display:none;}div[data-testid='stTextInput'] div[data-baseweb='input']{background-color:transparent!important;border:none!important;box-shadow:none!important;}</style>", unsafe_allow_html=True)
...
# 3B. USER DASHBOARD
    ...
    st.components.v1.html(f"<script>function copyRef() {{ const el = document.createElement('textarea'); el.value = '{reflink}'; document.body.appendChild(el); el.select(); document.execCommand('copy'); document.body.removeChild(el); alert('Referral Link Copied!'); }}</script><button onclick='copyRef()' style='width: 100%; background-color: #1c2128; color: #00ff88; border: 1px solid #00ff88; padding: 10px; border-radius: 8px; cursor: pointer; font-weight: bold;'>📋 COPY REFERRAL LINK</button>", height=60)
    ...
    for idx, item in enumerate(list(data.get('inv', []))):
        ...
        st.markdown(f"<div class='cap-card'>LIVE PROFIT: ₱{live_profit:,.2f}</div>", unsafe_allow_html=True)
        is_op = end_dt <= ph_now <= pull_out_end
        ca, cb = st.columns(2)
        if ca.button("CLAIM INTEREST", key=f"int_{idx}", disabled=not is_op):
            data['wallet'] += roi_total
            item['start_time'] = ph_now.isoformat()
            save(st.session_state.user, data)
            st.rerun()
        if cb.button("PULL OUT CAPITAL", key=f"pull_{idx}", disabled=not is_op):
...
# 3D. LANDING / COVER PAGE (DEFAULT)
    ...
    # Hidden Admin Dot (Color set to very dark to keep it 'ghost')
    st.markdown("<style>div.stButton > button[kind='secondary'] { background-color: transparent !important; border: none !important; color: #111111 !important; font-size: 8px !important; }</style>", unsafe_allow_html=True)
    if st.button("."): 
